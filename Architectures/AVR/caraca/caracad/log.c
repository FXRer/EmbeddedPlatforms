...
void  debug(int level,const char *fmt,...)
{
        va_list ap;
        va_start(ap, fmt);
        if (debug_level && level) vprintf(fmt, ap);
        va_end(ap);
}
 
static char *logstr[CARACAD_LOG_MAX] =
{
	"Fatal",
	"Debug",
	"Info",
...
static int log_enabled[CARACAD_LOG_MAX];
...
{
	if (level < CARACAD_LOG_MAX) log_enabled[level] = enabled;
}
...
void log(int level, char* format,...)
{
	...
	if ( !log_on || level >= CARACAD_LOG_MAX || !log_enabled[level]) return;
