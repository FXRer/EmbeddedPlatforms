 ...
 *--- Event and Action Table and Action routines ---*
 $Id: node_skr_action.s,v 1.1 2000/12/07 11:12:43 skr Exp $
...
public e2_SendEEprom:
            dc.w    BUILD_MSGID(27,DEFNODE_ADDR,0,2)
public e2_SendDebug:
...
csa_etloop:
            ...
            brne    csa_etloop
csa_end:
	    rjmp    canSendSerial        ;send Hello with new ID
            ;ret


/* Action: Set Dimmer */

#define TBright         Reg1    /* Range: 0(off) to 0x8c(max) */
;local Var
canSetDimmer:
	    ;1 Byte - use a lookup table to get 2 Byte Counter
            ld      TBright,Z+
            tst     TBright
            breq    LightOff
            cpi     TBright,0x8c        /* max Brightness */
            skipcs
            ldi     TBright,0x8c
            mov     Arg2,TBright
...
step_add:   add     Res2,Arg3
            ...
            sbrs    TriacBits,TOn
            ldi     TriacBits,BV(TSwitchOn)
            sei
            ret
LightOff:   cbr     TriacBits,BV(TOn)|BV(TSynced)|BV(TSwitchOn)
            ret
