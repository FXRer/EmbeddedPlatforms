# $Id$
"""Wibo host utility

Usage:
    wibohost.py -P <PORT> -a <ADDRESS> -c <CHANNELS> [-u FILE | -U FILE] [-S] [-J | -E]
    wibohost.py (-h | --help)

Options:
  -P --port=PORT     : port string ("port" or "port:baudrate"), e.g. /dev/ttyUSB0:38400 COM4:57600
  --version          : show version and exit
  -a --address=ADDR  : set current node addresses, e.g. -a 1, -a 1,3,5 or -a 1:10,15 [default 1:8]
  -u FILE        : selectively update nodes selected by ADDR with FILE
  -U FILE        : broadcast update nodes with FILE
  -S             : scan for nodes in range min(ADDR):max(ADDR),
  -J             : send jump_to_bootloader frame to nodes selected by ADDR
  -E             : send exit_from_bootloader frame to nodes selected by ADDR
  -c CHANS       : issue jump bootloader over the given channels, default: [11]
  -v             : increase verbose level
  -h             : show help and exit
"""

# Python 2 backwards compatibility
from __future__ import print_function

# Python standard distribution
# import getopt
import sys
import time
import re
import string

# external modules, installable via pip unlike otherwise mentioned
import serial
import docopt  # http://github.com/docopt


class WIBOHost(object):
    """ Command to interface to WIBO Host device via serial line """

    def __init__(self, emulation=False, *args, **kwargs):
        """
            Initialize serial port
        """
        if emulation is True:
            self._sendcommand = self._sendcommand_emulation
        else:
            self._sendcommand = self._sendcommand_serial
        self.sport = serial.Serial(*args, **kwargs)
        self.flt = re.compile("(?P<status>[A-Z]+)([ ]?)(?P<data>.*)")
        self.cmdcnt = 0

    def _print(self, verbose, *args, **kwargs):
        """
            Print with a given verbose level
        """
        if verbose <= self.VERBOSE:
            print(args, **kwargs)

    def _sendcommand_emulation(self, cmd, *args):

        map = {
            'ping': {'status': 'OK', 'data': "{'short_addr':0x0001, 'appname': 'wibo'," " 'boardname':'radiofaro', 'version':0x01, " "'crc':0x1234}"},
            'deaf': {'status': 'OK', 'data': {}},
            'finish': {'status': 'OK', 'data': {}},
            'feedhex': {'status': 'OK', 'data': {}},
            'reset': {'status': 'OK', 'data': {}},
            'exit': {'status': 'OK', 'data': {}},
            'crc': {'status': 'OK', 'data': {}},
            'echo': {'status': 'OK', 'data': {}},
            'info': {'status': 'OK', 'data': {}},
            'target': {'status': 'OK', 'data': {}},
            'jbootl': {'status': 'OK', 'data': {}},
            'bootlup': {'status': 'OK', 'data': {}},
            'channel': {'status': 'OK', 'data': {}},
            'panid': {'status': 'OK', 'data': {}},
            'shortaddr': {'status': 'OK', 'data': {}}
        }
        self.cmdcnt += 1

        return map[cmd]

    def _sendcommand_serial(self, cmd, *args):
        """
            Internal function
            Send command to device, wait response and evaluate it
        """
        self.cmdcnt += 1

        # Flush serial buffer, used before issuing command
        self.sport.read(self.sport.inWaiting())

        cmd = string.join(map(str, [cmd] + list(args)))
        self._print(3, "TX[%d]: %s" % (self.cmdcnt, cmd))
        self.sport.write(cmd + '\n')
        # TODO evaluate returning line and parse for parameters
        s = self.sport.readline().strip()
        self._print(3, "RX[%d]: %s" % (self.cmdcnt, s))
        m = self.flt.match(s)
        if m is None:
            return dict(status="NO RESPONSE", data=cmd)
        else:
            return m.groupdict()

    def ping(self, nodeid):
        """ Ping a dedicated node and evaluate reply """
        ret = self._sendcommand('ping', hex(nodeid))
        if ret['status'] == 'OK':
            ret['data'] = eval(ret['data'])
        return ret

    def deaf(self, nodeid):
        """ Set node to deaf """
        return self._sendcommand('deaf', hex(nodeid))

    def finish(self, nodeid):
        return self._sendcommand('finish', hex(nodeid))

    def feedhex(self, nodeid, ln):
        return self._sendcommand('feedhex', hex(nodeid), ln)

    def reset(self):
        return self._sendcommand('reset')

    def exit(self, nodeid):
        return self._sendcommand('exit', hex(nodeid))

    def crc(self):
        return self._sendcommand('crc')

    def echo(self, dstr):
        return self._sendcommand('echo', dstr)

    def info(self):
        ret = self._sendcommand('info')
        if ret['status'] == 'OK':
            ret['data'] = eval(ret['data'])
            return ret
        else:
            return None

    def target(self, nodeid, targ):
        """ Set memory target
            'E' : EEPROM
            'F' : Flash memory
            'X' : No memory, dry run
        """
        return self._sendcommand('target', hex(nodeid), targ)

    def jbootl(self, nodeid):
        """ Jump to bootloader """
        return self._sendcommand('jbootl', hex(nodeid))

    def bootlup(self, nodeid):
        """ Initiate Bootloader update """
        return self._sendcommand('bootlup', hex(nodeid))

    def channel(self, channel):
        """ Set my CHANNEL """
        return self._sendcommand('channel', hex(channel))

    def panid(self, pan_id):
        """ Set my PAN_ID """
        return self._sendcommand('panid', hex(pan_id))

    def shortaddr(self, short_addr):
        """ Set my SHORT_ADDR """
        return self._sendcommand('shortaddr', hex(short_addr))


class NodeList(list):
    """ Little helper class to pretty print a list of nodes in ascii """

    def __init__(self, *args, **kwargs):
        list.__init__(self, *args, **kwargs)

    def __repr__(self):
        keys = ['short_addr',
                'boardname',
                'appname',
                'status',
                'target',
                'version',
                ]  # to have the dict sorted
        keymap = {'short_addr': 'ADDR',
                  'boardname': 'BOARD',
                  'appname': 'APP',
                  'status': 'status',
                  'target': 'TARGET',
                  'version': 'VERSION',
                  }
        if self.__len__() == 0:
            ret = '-empty-'
        else:
            colwidth = {}
            [colwidth.update({k: len(keymap[k])}) for k in keys]
            for k in keys:
                for d in self.__iter__():
                    l = len(str(d[k]))
                    if l > colwidth[k]:
                        colwidth[k] = l
            [colwidth.update({k: v + 2}) for k, v in colwidth.iteritems()]

            ret = ''
            ret += 'IDX '
            for k in keys:
                ret += keymap[k] + ' ' * (colwidth[k] - len(keymap[k]))
            ret += '\n'
            for i, d in enumerate(self.__iter__()):
                ret += '%03d ' % i
                for k in keys:
                    s = str(d[k])
                    ret += s + ' ' * (colwidth[k] - len(s))
                ret += '\n'
        return ret


class WIBONetwork(WIBOHost):
    """ Class to represent a list of WIBO nodes """

    def __init__(self, emulation=None, *args, **kwargs):
        """ Constructor """
        WIBOHost.__init__(self, emulation=emulation, *args, **kwargs)
        self.nodes = NodeList()

    def ping(self, nodeid):
        defaults = {'status': None, 'target': 'F'}
        # yes, we need the function from WIBOHost
        ret = WIBOHost.ping(self, nodeid)
        if ret['status'] == 'OK':
            data = ret['data']
            if data['short_addr'] not in [i['short_addr'] for i in self.nodes]:
                data.update(defaults)
                self.nodes.append(data)
            else:
                [n.update(data) for n in self.nodes if n['short_addr']
                 == data['short_addr']]
        return ret

    def scan(self, scanrange=None):
        """ Scan a range of nodes by pinging them """
        if self.VERBOSE >= 1:
            self._print(1, "scan nodes", scanrange)

        if scanrange is not None:  # Scan given range of addresses
            for a in scanrange:
                tmp = self.ping(a)
                if tmp['status'] == 'OK':
                    self._print(2, "0x%04x" %
                                tmp['data']['short_addr'], end='')
                    if (len(self.nodes) % 4) == 0:
                        print("\n        ", end='')
                else:
                    self._print(2, "Already in list")
        else:  # Scan unknown address range
            lst = []
            self.reset()
            self.nodes = NodeList()
            retry = 0
            while retry < 5:
                tmp = self.ping(0xFFFF)  # broadcast
                if tmp['status'] == 'OK':
                    lst.append(tmp['data'])

                    # TODO: command not supported in earlier versions
                    self.deaf(tmp['data']['short_addr'])
                else:
                    retry += 1
        self._print(1, "\nFound %d devices\n" % len(self.nodes))

    def flashhex(self, nodeid, fname):
        """ Flash hex-file to nodeid (single node or broadcast) """
        f = open(fname)
        self.reset()
        for i, ln in enumerate(f):
            ret = self.feedhex(nodeid, ln.strip())
            if ret['status'] == 'ERR':
                self._print(0, 'ERR', ret['data'])
                break
            elif ret['status'] == 'WARN':
                self._print(0, 'WARN', ret['data'])
            self._print(1, "line %-4d crc: 0x%04x\r" %
                        (i, int(self.crc()['data'], 16)), end='')
            sys.stdout.flush()
            self._print(2, i, ln.strip(), self.crc()['data'])

        self.finish(nodeid)
        f.close()

    def checkcrc(self):
        """ Check CRC of node list and compare to the local CRC of host    """
        hostcrc = int(self.crc()['data'], 16)
        for n in self.nodes:
            p = self.ping(n['short_addr'])
            if p['status'] == 'OK':
                if (hostcrc == p['data']['crc']):
                    n['status'] = 'OK'
                else:
                    n['status'] = 'FAIL'
            else:  # no reply
                n['status'] = 'DISCONNECT'

    def filter_board(self, boardname):
        """ Set target to "Flash" for boards matching boardname, "X" (dry run) else """
        for n in self.nodes:
            if n['boardname'] == boardname:
                targ = 'F'
            else:
                targ = 'X'

            n['target'] = targ  # local list
            ret = self.target(n['short_addr'], 'F')  # write to device
            if ret['status'] != 'OK':
                raise Exception("Could not set target")


def param_evaluate_list(arg):
    lst = []
    for a in arg.split(","):
        a = a.split(":")
        if len(a) > 1:
            lst += eval("range(%s,%s+1)" % (a[0], a[-1]))
        else:
            lst.append(eval(a[0]))
    lst.sort()
    return lst


def process_command_line(**arguments):
    """Process the arguments, that were parsed by docopt from the __doc__ string on top
       of the file
    """
    ADDRESSES = range(8)
    VERBOSE = 0
    CHANNELS = None
    PORT = None
    BAUDRATE = None
    ret = False
    wnwkemulation = False

    # options containing parameters that are required to work
    for o, v in arguments.items():
        if o == "-v":
            VERBOSE += 1
        elif o == "--port":
            p, b = v.split(":")
            PORT = p
            try:
                BAUDRATE = int(b)
            except ValueError:
                raise ValueError("Invalid baudrate")
        elif o == "-a":
            ADDRESSES = param_evaluate_list(v)
        elif o == "-c":
            CHANNELS = param_evaluate_list(v)

    # secret key, attend the numbers!
    wnwkemulation = (PORT == '3MULATI0N' and BAUDRATE == 42)
    wnwk = WIBONetwork(emulation=wnwkemulation)

    if ret is False:
        wnwk.VERBOSE = VERBOSE
        if not wnwkemulation:
            wnwk.sport.close()
            settings = wnwk.sport.get_settings()
            settings['timeout'] = 1.0  # one second
            wnwk.sport.port = PORT  # no API function available (pyserial 3.3)
            settings['baudrate'] = BAUDRATE
            wnwk.sport.apply_settings(settings)
            wnwk.sport.open()
            time.sleep(0.1)  # some time for boot printout

        for i in range(10):
            x = wnwk.echo("HalloWibo")
            if x["status"] == "OK":
                break
            else:
                time.sleep(.1)
        if i == 9:
            raise Exception("FATAL", "unable to connect wibo host %s" % x)

        if arguments['-u'] is True:
            print("selective flashing nodes", ADDRESSES)
            for n in ADDRESSES:
                print("flash node", n)
                tmp = wnwk.ping(n)
                if tmp['status'] == 'OK' and tmp['data']['appname'] == "wibo":
                    wnwk.flashhex(n, v)
                    print("                      \r"
                          "file: %s, node: 0x%04x, crc: 0x%04x" %
                          (v, n, int(wnwk.crc()['data'], 16)))
                    print("Status Table")
                    wnwk.checkcrc()  # fetch status of each node in NWK...
                    print(wnwk.nodes)  # ...and print it
                    print("EXIT", n)
                else:
                    print("node %d is not responding" % n)

        if arguments['-U'] is True:
            if CHANNELS is None:
                wnwk.scan(ADDRESSES)
            else:
                for c in CHANNELS:
                    if VERBOSE:
                        print("\nchan %d:" % c,)
                    wnwk.channel(c)
                    wnwk.scan(ADDRESSES)
            ADDRESSES = [n['short_addr'] for n in wnwk.nodes]
            listeners = []
            for n in ADDRESSES:
                ret = wnwk.ping(n)
                if ret['data']['appname'] == "wibo":
                    listeners.append(n)
            if len(listeners) == 0:
                print("skip flashing, no listeners")
            else:
                print("broadcast flashing nodes:", listeners)
                wnwk.flashhex(0xffff, v)
                print("                      \r"
                      "file: %s, node: 0x%04x, crc: 0x%04x" %
                      (v, n, int(wnwk.crc()['data'], 16)))
                for n in listeners:
                    print("CRC:", n, wnwk.checkcrc())  # TODO: rework
                    for n in wnwk.nodes:
                        print(n['status'])

                    print("EXIT:", n)

        if arguments['-S'] is True:
            if CHANNELS == None:
                wnwk.scan(ADDRESSES)
            else:
                for c in CHANNELS:
                    if VERBOSE:
                        print("\nchan %d:" % c, end='')
                    wnwk.channel(c)
                    wnwk.scan(ADDRESSES)
            ADDRESSES = [n['short_addr'] for n in wnwk.nodes]
            if VERBOSE and len(wnwk.nodes):
                print(wnwk.nodes)
            print("SCAN:", ADDRESSES)

        if arguments['-J'] is True:
            if VERBOSE:
                print("Jump to bootloader", end='')
            if CHANNELS == None:
                for n in ADDRESSES:
                    if VERBOSE > 1:
                        print("\n node: ", n, end='')
                    wnwk.jbootl(n)
            else:
                for c in CHANNELS:
                    if VERBOSE > 1:
                        print("\nchannel: %d, node: " % c, end='')
                    wnwk.channel(c)
                    for n in ADDRESSES:
                        if VERBOSE > 1:
                            print(n, end='')
                        wnwk.jbootl(n)
            if VERBOSE:
                print(end='')  # line break
            listeners = []
            for n in ADDRESSES:
                res = wnwk.ping(n)
                if res['status'] == 'OK':
                    if res.get("appname") == "wibo":
                        listeners.append(n)
            print("Wibo listeners:", listeners)

        if arguments['-E'] is True:
            for n in ADDRESSES:
                wnwk.exit(n)
                print("EXIT:", n)
    # do not close serial port, let destructor of WiboNetwork do it
    return ret


if __name__ == "__main__":
    arguments = docopt.docopt(__doc__, version='Wibohost 1.0')
    do_exit = process_command_line(**arguments)
    
# EOF
